...
function ablesungBewerten(a) {
    ...
    const anstieg = typeof vorletzterWert === 'number' && vorletzterWert >= 0
        ? (wert - vorletzterWert) / wert : null;
    if (alterMs <= ABLESUNG_FRISCH_MS) {
        return { vollstaendig: true, alterMs, anstieg, grund: null };
    }
    if (alterMs > ABLESUNG_VERALTET_MS) {
        return {
            vollstaendig: false, alterMs, anstieg,
            grund: `letzte Ablesung ${Math.round(alterMs / 60000)} min vor Programmende - `
...
module.exports = { NACHLAUF_MS, STABIL_MAX, abfragenSinnvoll, nachlaufFortschreiben,
                   ENDSPURT_AB_MIN, ENDSPURT_TAKT_MS, ABLESUNG_FRISCH_MS, ABLESUNG_VERALTET_MS, ANSTIEG_ANTEIL,
                   ablesungBewerten, imEndspurt };
