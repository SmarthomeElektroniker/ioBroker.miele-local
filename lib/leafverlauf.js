...
function aufnehmen(bisher, leaf, felder, zeit) {
    ...
    for (const [feld, wert] of Object.entries(felder || {})) {
         ...
         */
        if (JSON.stringify(letzter) === JSON.stringify(wert)) continue;
        reihe.push([zeit, wert]);
        while (reihe.length > WECHSEL_JE_FELD) reihe.shift();
        je[feld] = reihe;
    }
    verlauf[leaf] = je;
    return verlauf;
...
function bewegt(verlauf, vonZeit, bisZeit) {
    ...
    for (const [leaf, felder] of Object.entries(verlauf || {})) {
        if (leaf === '_zustand') continue;      // kein Leaf, sondern der Geraetezustand
...
function zustandBei(verlauf, zeit) {
    ...
    for (const [t, z] of reihe) {
        if (t <= zeit) treffer = z; else break;
    }
    return treffer;
}

module.exports = { WECHSEL_JE_FELD, aufnehmen, bewegt, umfang,
                   zustandAufnehmen, zustandBei };
