...
/**
 * Eco-Werte aus einem ProcessData-Leaf. Als eigene Funktion, damit die Skalierung prüfbar ist
 * und nicht verstreut im Adapter steht: Energie kommt in Wh, Wasser in Zehntellitern.
 * Die Feldindizes sind modellabhängig (WCR860: 25 und 40) und werden von aussen gegeben.
 *
 * @returns {{energyWh: number|null, energyKwh: number|null, waterL: number|null}}
 */
function ecoValues(fields, energyIdx, waterIdx) {
    ...
    return {
        ...
        waterL: w == null ? null : Math.round(Number(w)) / 10,
    };
}

module.exports = { parseLeaf, interpValue, decodeField, ecoValues };
