 ...
 */
function discover(timeoutMs = 5000, log = () => {}) {
    return new Promise(resolve => {
        let multicastDns;
        try {
            multicastDns = require('multicast-dns');
        } catch (e) {
            log('mDNS-Discovery nicht verfügbar (multicast-dns nicht installiert) - bitte Geräte-IPs manuell eintragen.');
            return resolve([]);
        }
        ...
        query();
        const q2 = setTimeout(query, 1000);
        ...
        setTimeout(() => {
            ...
            log(`mDNS: ${list.length} Miele-Gerät(e) gefunden`);
            resolve(list);
        }, timeoutMs);
    });
}
