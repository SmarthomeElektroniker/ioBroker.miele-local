...
function beimEinschaltenStarten({ an, vorher, nachher, schalter, geprueft }) {
    if (!an) return false;
    if (Number(nachher) === 1) return false;                    // wird gerade ausgeschaltet
    if (vorher != null && Number(vorher) !== 1) return false;    // war schon an
    if (schalter === true) return false;                         // laeuft bereits
...
module.exports = { BEREICHE, PAUSE_MS, SCAN_TIMEOUT_MS, JE_DURCHGANG, SICHERN_ALLE,
                   ABBRUCH_FEHLER, ABSAGEN_JE_ADRESSE, ueberlastet, beantwortet, beschaeftigt,
                   wartezeitMs,
                   adressen, schluessel, naechste, aufnehmen, fortschritt, treffer,
                   unterschiede, beimEinschaltenStarten, trotzdemVersuchen };
