...
function programTypeTextDe(v) { const eigen = de.programTypeDe(v); return eigen != null ? eigen : enums.programTypeText(v); }
function dryingStepTextDe(v) { const eigen = de.dryingStepDe(v); return eigen != null ? eigen : enums.dryingStepText(v); }
/**
...
function zustandsTexte(sub, deviceType, german) {
    ...
    if (!schluessel.length || schluessel.length > STATES_MAX) return undefined;
    const aus = {};
...
function ecoCommon(german) {
    return {
        ...
        'eco.energyWh': {
            name: namen.text('Energie in Wh (Erwartung, keine Messung)',
                             'Energy in Wh (estimate, not measured)', german),
...
function ecoStates(german, rohfelder) {
    ...
    if (rohfelder) {
        liste.push({
            sub: 'felderJson',
            common: {
                name: namen.text('Alle Eco-Rohfelder (Fehlersuche)',
                                 'All raw eco fields (diagnostics)', german),
                role: 'json', type: 'string', read: true, write: false, def: '',
            },
        });
    }
    return liste;
...
module.exports = { STATE_FIELDS, zustandsTexte, IDENT_FIELDS, CONTROL_STATES, DE_NAMES, DEVICE_CATEGORY, deviceCategory, FIELD_DEVICE_TYPES, fieldAllowed, nameFor, timeToMinutes, temp, pathGet, TEMP_SENTINEL, ecoCommon, ecoStates };
