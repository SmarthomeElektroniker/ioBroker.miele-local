...
const STATE_FIELDS = {
    Status: {
        states: [
            { sub: 'status', role: 'value', type: 'number', name: 'Status (raw)' },
            { sub: 'statusText', role: 'text', type: 'string', name: 'Status' },
        ],
        decode: (v) => [{ sub: 'status', val: v }, { sub: 'statusText', val: statusTextDe(v) }],
    },
    ProgramType: {
        states: [
            { sub: 'programType', role: 'value', type: 'number', name: 'Program type (raw)' },
            { sub: 'programTypeText', role: 'text', type: 'string', name: 'Program type' },
        ],
        decode: (v) => [{ sub: 'programType', val: v }, { sub: 'programTypeText', val: programTypeTextDe(v) }],
    },
    ProgramID: {
        states: [
            { sub: 'programId', role: 'value', type: 'number', name: 'Program (raw)' },
            { sub: 'programText', role: 'text', type: 'string', name: 'Program' },
        ],
        decode: (v, ctx) => [{ sub: 'programId', val: v }, { sub: 'programText', val: programTextDe(ctx.deviceType, v) }],
    },
    ProgramPhase: {
        states: [
            { sub: 'programPhase', role: 'value', type: 'number', name: 'Phase (raw)' },
            { sub: 'programPhaseText', role: 'text', type: 'string', name: 'Phase' },
        ],
        decode: (v, ctx) => [{ sub: 'programPhase', val: v }, { sub: 'programPhaseText', val: phaseTextDe(ctx.deviceType, v) }],
    },
    RemainingTime: {
        states: [
            { sub: 'remainingMinutes', role: 'value.interval', type: 'number', unit: 'min', name: 'Remaining time' },
            { sub: 'remainingHHMM', role: 'text', type: 'string', name: 'Remaining time (H:MM)' },
            ...
            { sub: 'estimatedEndTime', role: 'value.time', type: 'number', name: 'Estimated end time' },
            { sub: 'estimatedEndTimeText', role: 'text', type: 'string', name: 'Estimated end time (HH:MM)' },
        ],
        decode: (v) => [{ sub: 'remainingMinutes', val: timeToMinutes(v) }, { sub: 'remainingHHMM', val: timeToHHMM(v) }],
    },
    ElapsedTime: {
        states: [
            { sub: 'elapsedMinutes', role: 'value.interval', type: 'number', unit: 'min', name: 'Elapsed time' },
            { sub: 'elapsedHHMM', role: 'text', type: 'string', name: 'Elapsed time (H:MM)' },
        ],
        decode: (v) => [{ sub: 'elapsedMinutes', val: timeToMinutes(v) }, { sub: 'elapsedHHMM', val: timeToHHMM(v) }],
    },
    StartTime: {
        states: [
            { sub: 'startInMinutes', role: 'value.interval', type: 'number', unit: 'min', name: 'Start delay' },
            { sub: 'startHHMM', role: 'text', type: 'string', name: 'Start delay (H:MM)' },
        ],
        decode: (v) => [{ sub: 'startInMinutes', val: timeToMinutes(v) }, { sub: 'startHHMM', val: timeToHHMM(v) }],
    },
    TargetTemperature: {
        states: [
            { sub: 'targetTemperature', role: 'value.temperature', type: 'number', unit: '°C', name: 'Target temperature' },
            { sub: 'targetTemperatureZone2', role: 'value.temperature', type: 'number', unit: '°C', name: 'Target temperature zone 2' },
            { sub: 'targetTemperatureZone3', role: 'value.temperature', type: 'number', unit: '°C', name: 'Target temperature zone 3' },
        ],
        decode: (v) => [
    ...
    Temperature: {
        states: [
            { sub: 'temperature', role: 'value.temperature', type: 'number', unit: '°C', name: 'Temperature' },
            { sub: 'temperatureZone2', role: 'value.temperature', type: 'number', unit: '°C', name: 'Temperature zone 2' },
            { sub: 'temperatureZone3', role: 'value.temperature', type: 'number', unit: '°C', name: 'Temperature zone 3' },
        ],
        decode: (v) => [
    ...
    SignalInfo: {
        states: [{ sub: 'signalInfo', role: 'indicator', type: 'boolean', name: 'Info signal' }],
        decode: (v) => [{ sub: 'signalInfo', val: !!v }],
    },
    SignalFailure: {
        states: [{ sub: 'signalFailure', role: 'indicator.maintenance', type: 'boolean', name: 'Failure signal' }],
        decode: (v) => [{ sub: 'signalFailure', val: !!v }],
    },
    SignalDoor: {
        states: [{ sub: 'signalDoor', role: 'sensor.door', type: 'boolean', name: 'Door open' }],
        decode: (v) => [{ sub: 'signalDoor', val: !!v }],
    },
    RemoteEnable: {
        states: [
            { sub: 'mobileStart', role: 'indicator', type: 'boolean', name: 'MobileStart enabled' },
            { sub: 'remoteEnableRaw', role: 'json', type: 'string', name: 'RemoteEnable (raw)' },
        ],
    ...
    ProcessAction: {
        states: [{ sub: 'processAction', role: 'value', type: 'number', name: 'Process action' }],
        decode: (v) => [{ sub: 'processAction', val: v }],
    },
    DeviceAction: {
        states: [{ sub: 'deviceAction', role: 'value', type: 'number', name: 'Device action' }],
        decode: (v) => [{ sub: 'deviceAction', val: v }],
    },
    Light: {
        states: [{ sub: 'light', role: 'switch.light', type: 'boolean', name: 'Light on' }],
        // Light: 0 = kein Licht/aus, 1 = an, 2 = aus (gerätespezifisch); >0 && !=2 → an
        decode: (v) => [{ sub: 'light', val: v === 1 }],
    },
    StandbyState: {
        states: [{ sub: 'standbyState', role: 'value', type: 'number', name: 'Standby state' }],
        decode: (v) => [{ sub: 'standbyState', val: v }],
    },
    SpinningSpeed: {
        states: [{ sub: 'spinningSpeed', role: 'value', type: 'number', unit: 'rpm', name: 'Spin speed' }],
        decode: (v) => [{ sub: 'spinningSpeed', val: v }],
    },
    DryingStep: {
        states: [
            { sub: 'dryingStep', role: 'value', type: 'number', name: 'Drying step (raw)' },
            { sub: 'dryingStepText', role: 'text', type: 'string', name: 'Drying step' },
        ],
        decode: (v) => [{ sub: 'dryingStep', val: v }, { sub: 'dryingStepText', val: dryingStepTextDe(v) }],
    },
    SyncState: {
        states: [{ sub: 'syncState', role: 'value', type: 'number', name: 'Sync state' }],
        decode: (v) => [{ sub: 'syncState', val: v }],
    },
    InternalState: {
        states: [{ sub: 'internalState', role: 'value', type: 'number', name: 'Internal state' }],
...
const IDENT_FIELDS = [
    { sub: 'techType', role: 'info.name', type: 'string', name: 'Model / TechType', path: ['DeviceIdentLabel', 'TechType'] },
    { sub: 'fabNumber', role: 'info.serial', type: 'string', name: 'Serial number', path: ['DeviceIdentLabel', 'FabNumber'] },
    { sub: 'matNumber', role: 'text', type: 'string', name: 'Material number', path: ['DeviceIdentLabel', 'MatNumber'] },
    { sub: 'deviceType', role: 'value', type: 'number', name: 'Device type', path: ['DeviceType'] },
    { sub: 'xkmType', role: 'text', type: 'string', name: 'WiFi module type', path: ['XKMIdentLabel', 'TechType'] },
    { sub: 'xkmVersion', role: 'info.firmware', type: 'string', name: 'WiFi module firmware', path: ['XKMIdentLabel', 'ReleaseVersion'] },
    { sub: 'protocolVersion', role: 'value', type: 'number', name: 'Protocol version', path: ['ProtocolVersion'] },
];

/** Steuer-States (beschreibbar, per Opcode). Nur angelegt, wenn allowControl. */
const CONTROL_STATES = [
    { sub: 'start', role: 'button.start', name: 'Start program', opcode: 0x01 },
    { sub: 'stop', role: 'button.stop', name: 'Stop program', opcode: 0x37 },
    { sub: 'pause', role: 'button.pause', name: 'Pause program', opcode: 0x03 },
    { sub: 'powerOn', role: 'button', name: 'Switch on', opcode: 0x10 },
    { sub: 'powerOff', role: 'button', name: 'Switch off', opcode: 0x13 },
    { sub: 'lightOn', role: 'button', name: 'Light on', opcode: 0x0d },
    { sub: 'lightOff', role: 'button', name: 'Light off', opcode: 0x0e },
];
