...
const NICHT_ANLEGEN = new Set([
    'DeviceIdentStrings', 'FileList', 'FileInfo', 'FileWrite', 'FileTransfer', 'RsaKey',
    'HeisluftPlus', 'NotificationAcknowledge', 'UserRequest', 'XkmRequest',
]);
 ...
 */
const DEUTSCH = {
    'Process.4': 'Restzeit', 'Process.5': 'Programmphase', 'Process.8': 'Heizrelais',
    'Process.15': 'Temperatur am Frequenzumrichter', 'Process.21': 'Wasser (Durchflusszähler)',
    'Process.24': 'Zieltemperatur der Heizung', 'Process.25': 'Heizenergie',
    'Process.26': 'Heizzeit', 'Process.27': 'Trommeldrehzahl', 'Process.65': 'Beladung',
    'ActuatorData.1': 'Heizung 1', 'ActuatorData.7': 'Heizung 2',
    'ActuatorData.2': 'Laugenpumpe', 'ActuatorData.3': 'Intensivpumpe',
    'Sensor.1': 'Wasserstand', 'Sensor.3': 'Schleuderdrehzahl', 'Sensor.4': 'Türschalter',
    ...
    'DeviceState.7': 'Restzeit', 'DeviceState.8': 'Laufzeit',
    'DeviceCombiState.1': 'Gerätezustand', 'DeviceCombiState.2': 'Betriebszustand',
    ...
    for (const [idx, f] of Object.entries(fields || {})) {
        ...
        if (kind && f.type === dop2.TYP_STRUKTUR && Array.isArray(f.value)) {
            ...
            for (const e of f.value) {
                if (e && typeof e === 'object' && typeof e.id === 'number') innen[e.id] = e;
            }
            if (Object.keys(innen).length) {
                for (const u of ausStruktur(kind, innen, `${praefix}${nr}.`)) aus.push(u);
                continue;
            }
        }
        const wert = f.type === dop2.TYP_STRUKTUR && Array.isArray(f.value)
...
function kanalFuer(struktur, leaf) {
    if (KANAELE[struktur]) return KANAELE[struktur];
    const traeger = leafnamen.strukturVon(leaf);
    if (traeger && KANAELE[traeger]) return KANAELE[traeger];
...
function typVon(wert) {
    if (typeof wert === 'boolean') return { type: 'boolean', role: 'indicator', def: false };
    if (typeof wert === 'number') return { type: 'number', role: 'value', def: 0 };
    if (typeof wert === 'string') return { type: 'string', role: 'text', def: '' };
    if (Array.isArray(wert)) return { type: 'string', role: 'json', def: '' };
    return null;
}
...
function definition(leaf, struktur, idx, wert, deutsch) {
    if (!struktur || NICHT_ANLEGEN.has(struktur)) return null;
    ...
    let val = wert;
    if (typ.type === 'string' && typ.role === 'json') val = JSON.stringify(wert);
    if (typ.type === 'number' && extra.teiler) {
        val = Math.round((Number(wert) / extra.teiler) * 100) / 100;
    }

    const beschriftung = DEUTSCH[schluessel]
        ? namen.text(DEUTSCH[schluessel], name, deutsch)
        : name;
    const common = {
        name: beschriftung,
        role: (typ.type === 'number' && extra.rolle) || typ.role,
        type: typ.type,
        read: true, write: false, def: typ.def,
    };
    if (extra.einheit && typ.type === 'number') common.unit = extra.einheit;
...
module.exports = { KANAELE, KANAL_NAMEN, NICHT_ANLEGEN, BESONDERS, DEUTSCH,
                   definition, fuerLeaf, istwerte, kanalFuer, kanalName, typVon };
