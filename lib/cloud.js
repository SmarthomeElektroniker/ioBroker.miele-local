...
function buildAuthorizeUrl(cc) {
    ...
    if (!clientId) {
        throw new Error(`Unbekanntes Land "${cc}". Unterstützt: ${Object.keys(CONSUMER_CLIENT_IDS).join(', ')}`);
    }
...
function parseRedirectUrl(redirectUrl, expectedState) {
    ...
    if (parsed.get('error')) {
        throw new Error(`OAuth-Fehler: ${parsed.get('error')} ${parsed.get('error_description') || ''}`);
    }
    const code = parsed.get('code');
    if (!code) {
        throw new Error('Kein "code" in der eingefügten Redirect-URL.');
    }
    const state = parsed.get('state');
    if (expectedState && state !== expectedState) {
        throw new Error('state stimmt nicht überein (CSRF-Schutz) – bitte Login-URL neu erzeugen.');
    }
    return code;
}
...
async function exchangeCode(challenge, code) {
    ...
    try {
        ...
        throw new Error(`Nicht-JSON-Token-Antwort: ${res.body.slice(0, 200)}`);
    }
    if (tok.error) {
        throw new Error(`Token-Endpoint: ${tok.error} ${tok.error_description || ''}`);
    }
    return tok;
}
...
async function refreshAccessToken(cc, refreshToken) {
    ...
    if (tok.error) {
        throw new Error(`Refresh fehlgeschlagen: ${tok.error} ${tok.error_description || ''}`);
    }
    return tok;
}
...
async function fetchGroupKey(accessToken, region = 'EU') {
    ...
    if (res.status === 403) {
        throw new Error(`GroupKeyId → 403 (Token ohne mcs-Scope?). ${res.body.slice(0, 150)}`);
    }
    if (res.status !== 200) {
        throw new Error(`GroupKeyId → HTTP ${res.status}: ${res.body.slice(0, 150)}`);
    }
    const groups = JSON.parse(res.body);
    if (!groups || !groups.length) {
        throw new Error('Kein Haushalt zurückgegeben – Konto ohne gekoppelte Geräte?');
    }
    const g = groups[0];
