...
function vergleichswert(satz, groesse) {
     ...
     */
    if (satz.unvollstaendig && satz.unvollstaendig[groesse]) return null;
    /*
    ...
    if (groesse === 'energyKwh') {
        ...
        if (wh != null && wh > 0) return { wert: wh / 1000, quelle: 'gemessen' };
    }
    ...
    for (const [groesse, beschriftung, gesetzt] of [
        ...
        if (!beste.length) {
            ...
            zeilen.push(`${beschriftung}: kein Feld passt zu den ${n} Vergleichswerten`
                + (nah ? ` (am naechsten Feld ${benannt(nah.index)} mit ${(nah.abweichung * 100).toFixed(0)} % Abweichung)` : '')
                + '.');
            continue;
        }
        const b = beste[0];
